...
require(mxnet)
#setwd("I:/Desktop/R/LSTnet - anomaly/src/")
setwd('~/Documents/deeplearning.mxnet/LSTnet-anomaly/src/')
source("./lstnet_anomaly_train.R")
                             ...
                             max.records = NULL)
{
  ...
  train <- list(data=as.array(x.train), label=rep(1,train_samples))
  valid <- list(data=as.array(x.valid), label=rep(1,valid_samples))
  test  <- list(data=as.array(x.test),  label=rep(1,test_samples))
