...
mx.rnn.lstnet.anomaly.skip <- function(seasonal.period,
                               time.interval,
                               filter.list,
                               num.filter,
                               seq.len,
                               ...
                               dropout = 0)
{
  ...
  R2 <- mx.symbol.Variable('param.R2')
  a <- mx.symbol.Variable('param.centre')
  ...
  svdd.input <- mx.symbol.BlockGrad(model.output, name='svdd')
  
  #############
  # SVDD LOSS #  
  #############
  radius.dist <- mx.symbol.sum(mx.symbol.square(mx.symbol.broadcast_minus(model.output,a)), axis=1)
  radius.diff <- mx.symbol.broadcast_minus(radius.dist, R2)
  hinge.loss <- mx.symbol.log(mx.symbol.exp(radius.diff*tau)+1)/tau
  sum.s <- mx.symbol.sum(hinge.loss)
  loss <- R2+sum.s*C
  loss.svdd <- mx.symbol.MakeLoss(loss, name='loss')
  ...
  #include everything
  list.all <- c(loss.svdd, gru.unpack.h, lstm.unpack.c, lstm.unpack.h, svdd.input)
  return(mx.symbol.Group(list.all))
}

mx.rnn.lstnet.anomaly.attn <- function(filter.list,
                               num.filter,
                               seq.len,
                               input.size,
                               batch.size,
                               ...
                               dropout = 0)
{
  ...
  radius.dist <- mx.symbol.sum(mx.symbol.square(mx.symbol.broadcast_minus(model.output,a)), axis=1)
  radius.diff <- mx.symbol.broadcast_minus(radius.dist, R2)
  hinge.loss <- mx.symbol.log(mx.symbol.exp(label*radius.diff*tau)+1)/tau
  sum.s <- mx.symbol.sum(hinge.loss)
  loss <- R2+sum.s*C
  loss.svdd <- mx.symbol.MakeLoss(loss, name='loss')
  ...
  #include everything
  list.all <- c(loss.svdd, gru.unpack.h, svdd.input)
  return(mx.symbol.Group(list.all))
}