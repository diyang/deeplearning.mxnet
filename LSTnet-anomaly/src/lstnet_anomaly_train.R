...
require(mxnet)
source("./lstnet_anomaly_model.R")
#source("./metrics.R")
source("../../SVDD/src/svdd_dual.R")
...
train.lstnet.anomaly <- function(model,
                                 train.data,
                                 valid.data,
                                 num.epoch,
                                 learning.rate,
                                 ...
                                 lr_scheduler = NULL)
{
  ...
  #initialise SVDD parameters
  init.svdd <- list()
  init.svdd[['param.R2']] <- mx.nd.array(0.5)
  init.svdd[['param.centre']] <- mx.nd.array(matrix(rep(0, input.size))) 
  mx.exec.update.arg.arrays(m$lstnet.exec, init.svdd, match.name = TRUE)
  ...
  for(epoch in 1:num.epoch)
  {
    ...
    batch.counter <- 1
    param.R2 <- 0
    param.centre <- matrix(rep(0,input.size))
    ...
    while(train.data$iter.next()){
      ...
      #prepare SVDD input
      svdd.input <- m$lstnet.exec$ref.outputs[['svdd_output']]
      svdd.label <- train.input$label
      
      svdd.model<-svdd.smo(data = as.array(svdd.input), 
                           label = as.array(svdd.label),
                           C = 1e03, 
                           tolerant= 0.0001,
                           epsilon = 0.0001,
                           kernel='linear')
      
      svdd.model <- svdd.smo.train(svdd.model, epoch.max=10)
      param.R2 <- param.R2 + svdd.model$R2
      param.centre <- param.centre + as.array(svdd.input)%*%(svdd.model$alphas*svdd.model$label)
      
      # update SVDD vector a and radius
      init.svdd <- list()
      param.avg.R2 <- param.R2/batch.size
      init.svdd[['param.R2']] <- mx.nd.array(param.avg.R2[1])
      init.svdd[['param.centre']] <- mx.nd.array(matrix(param.centre/batch.size))
      mx.exec.update.arg.arrays(m$lstnet.exec, init.svdd, match.name = TRUE)
      ...
      train.loss <- m$lstnet.exec$ref.outputs[['loss_output']]
      cat(paste0("Epoch [", epoch, "] Batch [", batch.counter, "] Trianing Loss: ", as.array(train.loss),"\n"))
    ...
    if( !is.null(valid.data)){
      ...
      while (valid.data$iter.next()) {
        ...
        # evaluate performance and display
        valid.loss <- m$lstnet.exec$ref.outputs[['loss_output']]
        cat(paste0("Epoch [", epoch, "] Batch [", batch.counter, "] validation Loss:",as.array(valid.loss), " \n"))
        batch.counter <- batch.counter+1
      }
      valid.data$reset()
    }
...
lstnet.anomaly.setup.model <- function(lstnet.sym, 
                                       ctx = mx.ctx.default(), 
                                       num.rnn.layer = 1,
                                       num.filter,
                                       filter.list,
                                       ...
                                       dropout = 0)
{
  ...
  for (name in arg.names) {
    ...
    else if (grepl('R2$', name) ){
      input.shape[[name]] <- c(1)
    }
    else if (grepl('centre$', name) ){
      input.shape[[name]] <- c(input.size, 1)
    }
  }
                      ...
                      )
{
  ...
  if(type == 'skip'){
    lstnet.sym <- mx.rnn.lstnet.anomaly.skip(seasonal.period = seasonal.period,
    ...
    lstnet.sym <- mx.rnn.lstnet.anomaly.attn(filter.list = filter.list,
                                             num.filter = num.filter,
                                             seq.len = seq.len,
                                             input.size = input.size,
                                             batch.size = batch.size,
  ...
  model <- lstnet.anomaly.setup.model(lstnet.sym = lstnet.sym,
                                      ctx = ctx,
                                      num.rnn.layer = num.rnn.layer,
                                      num.filter = num.filter,
                                      filter.list = filter.list,
  ...
  model <- train.lstnet.anomaly(model,
